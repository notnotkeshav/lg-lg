...
class CRMQuotation(Document):
    ...
    def on_update(self):
        ...
        if (
            self.workflow_state == "Customer Approval Pending"
    ...
    def get_quotation_email_subject(self):
        ...
        return _("Quotation {0} - {1}AMC Proposal for {2}").format(
            self.name, amc_type, customer
        )
    ...
    def get_quotation_email_body(self):
        ...
        total = (
            flt(self.rounded_total_inr) or flt(self.grand_total_inr) or flt(self.amount)
        )
        ...
        rows = [
            ...
            (
                _("Quotation Date"),
                frappe.utils.formatdate(self.date, "dd MMMM yyyy")
                if self.date
                else "-",
            ),
            ...
            (
                _("Contract Dates"),
                "{0} to {1}".format(
                    frappe.utils.formatdate(self.start_date, "dd MMMM yyyy")
                    if self.start_date
                    else "-",
                    frappe.utils.formatdate(self.end_date, "dd MMMM yyyy")
                    if self.end_date
                    else "-",
                ),
            ),
            (
                ...
                f"{fmt_money(total, currency=currency)} ({_('inclusive of applicable taxes')})",
            ),
        ]
        ...
        if self.valid_till:
            rows.append(
                (
                    _("Offer Valid Till"),
                    frappe.utils.formatdate(self.valid_till, "dd MMMM yyyy"),
                )
            )
    ...
    def email_quotation_to_customer(self, recipients=None):
        ...
        if not recipients:
            frappe.msgprint(
                _(
                    "No customer email found on the quotation or on {0}, quotation not sent."
                ).format(self.customer or _("the organization")),
                indicator="orange",
                alert=True,
            )
            return
        ...
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(), f"Quotation Email Failed: {self.name}"
            )
            frappe.msgprint(
    ...
    sections = [
        {
            ...
            "fields": [
                f
                for f in fields
                if f.fieldtype in ["Link", "Data"]
                and f.fieldname in ["customer", "customer_hc"]
            ],
        },
        {
            "label": _("Quotation Details"),
            "fields": [
                f for f in fields if f.fieldname in ["date", "valid_till", "status"]
            ],
        },
        {
            ...
            "fields": [
                f
                for f in fields
                if f.fieldname in ["deal_type", "serial_no", "product_name"]
            ],
        },
        {
            ...
            "fields": [
                f
                for f in fields
                if f.fieldname in ["amc", "payment_frequency", "currency", "amount"]
            ],
        },
    ]
...
def update_field_layout(fieldname, key, value):
    ...
    docfield = frappe.get_doc(
        "DocField", {"parent": "CRM Quotation", "fieldname": fieldname}
    )
...
def export_report(filters=None):
    ...
    try:
        ...
        file_url = frappe.utils.get_site_path(
            "private", "files", "quotation_report.xlsx"
        )
...
@frappe.whitelist()
def get_zone_from_vertical_master(
    horse_power, project_type=None, start_date=None, end_date=None, price_rate=None
):
    if not project_type:
        return {"error": "Industry (Project Type) is required."}

    industry_doc = frappe.db.get_value(
        "CRM Industry", project_type, "parent_vertical", as_dict=True
    )
    ...
    for row in vertical_doc.hp_and_zone_details:
        try:
            ...
            frappe.log_error(
                f"Error parsing HP range '{row.hp}': {e}",
                "CRM Quotation: get_zone_from_vertical_master",
            )
    ...
    for q in quotations:
        ...
        unpaid_rows = sorted(
            [
                ...
                if row.status.lower() != "Paid" and row.payment_date
            ],
...
def send_advance_reminder(contract_name, owner, days_due, advance_end_date):
    print(
        f"📧 Triggering email reminder for: {contract_name}, Owner: {owner}, Days Due: {days_due}, Due Date: {advance_end_date}"
    )
    ...
    try:
        ...
        print(error_msg)
        frappe.log_error(
            title="Failed to Send Email Reminder", message=frappe.get_traceback()
        )
        return error_msg
...
def mark_or_notify_discontinuation(doc):
    ...
    if doc.custom_contract_status != "Discontinue":
        frappe.db.set_value(
            "CRM Contract",
            doc.name,
            {"custom_contract_status": "Discontinue", "discontinue_date": today},
        )
        frappe.db.commit()
...
def send_zone_approval_alert_api(docname, doctype="CRM Quotation"):
    ...
    try:
        ...
        if not role:
            frappe.log_error(
                f"Workflow state '{doc.workflow_state}' not in zone list",
                "Zone Approval Notification",
            )
        ...
        user_emails = [
            u["parent"] for u in recipients if u["parent"] != "Administrator"
        ]

        if not user_emails:
            frappe.log_error(
                f"No recipients found for role: {role}", "Zone Approval Notification"
            )
